	...
	for line in gtf:
		...
		if line.startswith("#"):
			...
			if len(gene_id) > 0:
				...
				gene_id = 'NA'
			#trans_id = [anno_lst for anno_lst in anno_lst if 'transcript_id ' in anno_lst]		#will be new ID
			...
			if len(int_num) > 0:
				...
				int_num = 'NA'
			#int_id = [anno_lst for anno_lst in anno_lst if 'exon_id ' in anno_lst]				#will be new ID
			...
			#	gene_tp = gene_tp.pop().split(' ')[1].strip('"')
...
with open(bed_file) as bed:
	...
	for line in bed:
		...
		VARS = [CHR,sorc_b,feat_b,START,STOP,SCORE,STR,frame,attr]
		...
		out_GTF.write('\t'.join(VARS)+';'+'\n')

print("Matching Genes: ",match)
print("Non Matching Genes: ",no_match)
out_GTF.close()
